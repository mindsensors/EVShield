 ...
 */
  imu.readGyro(mygyro);
  sprintf (str, "gyro      x:%d | y:%d | z:%d", mygyro.gx, mygyro.gy, mygyro.gz);
  Serial.println(str);
/**  Displays Tilt reading
 */  
  imu.readAccelerometer(myaccl);
  sprintf (str, "tilt      x:%d | y:%d | z:%d", myaccl.tx, myaccl.ty, myaccl.tz);
  Serial.println(str);
/**  Displays Accelerometer reading
 */  
  sprintf (str, "accl      x:%d | y:%d | z:%d", myaccl.ax, myaccl.ay, myaccl.az);
  Serial.println(str);
/**  Displays Magnetic Field reading
 */
  imu.readMagneticField(mymgnt);
  sprintf (str, "magfield  x:%d | y:%d | z:%d", mymgnt.mx, mymgnt.my, mymgnt.mz);
  Serial.println(str);
/**  Displays Compass reading
 */
  imu.readCompass(mycmps);
  sprintf (str, " compass:  %d", mycmps.heading );
  Serial.println(str);
  sprintf (str, "error:  %d", mycmps.error );
  Serial.println(str);
